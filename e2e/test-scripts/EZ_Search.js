...
module.exports = {
   ...
   // If a test suite is using a shared login for test scripts etc you can use the before hook to accomplish this before the tests are run or to setup specifics for tests
   ...
   'TESTING!!! Search results by keyword' : (client) => { // title of test script
      const search = client.page.searchPage(); // reference to call PO function from searchPage
   ...
   'TESTING!!! Multiple search results by keyword' : (client) => {
      ...
      // var items = [ // an array of text values for search result counts (moved to constants)
      //    {count: "Showing 1 - 7 of 7 items"},
      //    {count: "Showing 1 - 1 of 1 item"},
      //    {count: "Showing 1 - 1 of 1 item"},
      //    {count: "0 results have been found."}
      // ];
      //    for(i = 0; 1 <= x.length -1; i++){ // initial long form script to check multiple search result counts + debug logs 
